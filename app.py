...
def generate_image_report(data, images, bg_path, font_path):
    ...
    # บรรทัดที่ 3: ประเภท
    draw.text((start_x, start_y + gap*1.5), data["{{TYPE}}"], font=font_text, fill=text_color)
    
    # บรรทัดที่ 4: ผู้ควบคุม
    draw.text((start_x, start_y + gap*1.5), data["{{COMMANDER}}"], font=font_text, fill=text_color)
