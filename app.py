...
def generate_image_report(data, images, bg_path, font_path):
    ...
    # บรรทัดที่ 3: ประเภท
    draw.text((start_x, start_y + gap*2.3), data["{{TYPE}}"], font=font_text, fill=text_color)
    ...
    draw.text((start_x, start_y + gap*4.4), data["{{RISK}}"], font=font_text, fill=text_color)
    
    # บรรทัดที่ 6: ตรวจยานพาหนะ
    draw.text((start_x, start_y + gap*5.6), data["{{VEHICLE}}"], font=font_text, fill=text_color)
